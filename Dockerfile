...
ENTRYPOINT dbt deps && dbt build --target=$TARGET && dbt build -s elementary --target=$TARGET && edr monitor --profile-target=$TARGET --slack-token '$SLACK_TOKEN' --slack-channel-name temp-alerts